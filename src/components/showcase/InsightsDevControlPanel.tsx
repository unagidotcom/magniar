...
import { Monitor, Smartphone, Tablet, Settings2, BookOpen } from 'lucide-react';

interface InsightsDevControlPanelProps {
  currentViewport: ViewportMode;
  onViewportChange: (viewport: ViewportMode) => void;
  activeViewMode: 'homepage' | 'insights-page' | 'insights-detail';
  onViewModeChange: (mode: 'homepage' | 'insights-page' | 'insights-detail') => void;
  activeSlug: string;
  onSlugChange: (slug: string) => void;
}

export function InsightsDevControlPanel({
  currentViewport,
  onViewportChange,
  activeViewMode,
  onViewModeChange,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-[#F5F7FA] uppercase tracking-wider text-[11px]">
            CHAPTER 09 — INSIGHTS / INTELLIGENCE DEV CONTROL PANEL
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            <button
              onClick={() => onViewModeChange('insights-page')}
              className={`px-2.5 py-1.5 rounded-[2px] border text-[11px] cursor-pointer ${
                activeViewMode === 'insights-page'
                  ? 'bg-[#B89A72] text-white border-[#B89A72] font-semibold'
                  : 'bg-[#050505] text-[#8D949E] border-white/10 hover:text-white'
              }`}
            >
            ...
            <button
              onClick={() => onViewModeChange('insights-detail')}
              className={`px-2.5 py-1.5 rounded-[2px] border text-[11px] cursor-pointer ${
                activeViewMode === 'insights-detail'
                  ? 'bg-[#B89A72] text-white border-[#B89A72] font-semibold'
                  : 'bg-[#050505] text-[#8D949E] border-white/10 hover:text-white'
              }`}
            >
        ...
        <div>
          <label className="text-[#5A626E] text-[10px] uppercase block mb-1.5 font-bold">
            ARTICLE RECORD SELECTOR
          </label>
          <select
            value={activeSlug}
            onChange={(e) => {
              onSlugChange(e.target.value);
              onViewModeChange('insights-detail');
            }}
          ...
          >
            {INSIGHTS_ARTICLES_DATA.map((art) => (
              <option key={art.id} value={art.slug}>
                [{art.contentType}] {art.title}
              </option>
            ))}
          </select>
        </div>
