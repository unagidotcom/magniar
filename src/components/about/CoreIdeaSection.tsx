      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 relative">
          {steps.map((step, idx) => (
            <div
              key={step.num}
              className="p-6 bg-[#050505] border border-white/10 relative flex flex-col justify-between hover:border-[#0099FF]/50 transition-colors group"
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between font-mono text-xs">
                  <span className="text-[#0099FF] font-bold">{step.num}</span>
                  <span className="text-[#8D949E] text-[10px] uppercase tracking-wider">{step.label}</span>
                </div>
                <h3 className="text-base font-bold text-[#F5F7FA] group-hover:text-[#0099FF] transition-colors">
                  {step.title}
                </h3>
              ...
              {idx < steps.length - 1 && (
                <div className="hidden md:block absolute -right-2.5 top-1/2 -translate-y-1/2 z-10 text-[#0099FF]">
                  →
                </div>
              )}
            </div>
        ...
        {/* Highlight Banner */}
        <div className="mt-12 p-6 sm:p-8 bg-gradient-to-r from-[#0099FF]/10 via-transparent to-transparent border border-[#0099FF]/30 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            <Layers className="w-6 h-6 text-[#0099FF] shrink-0 mt-1" />
            <div>
          ...
          <button
            ...
            className="px-5 py-3 bg-[#0099FF] text-white font-mono text-xs font-bold uppercase tracking-wider hover:bg-[#0088EE] transition-all flex items-center gap-2 whitespace-nowrap self-start sm:self-center cursor-pointer"
          >
