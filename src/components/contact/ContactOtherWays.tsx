...
export const ContactOtherWays: React.FC = () => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'Mail':
        return <Mail className="w-5 h-5 text-[#0099FF]" />;
      case 'Clock':
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {CONTACT_OTHER_WAYS.map((way, idx) => (
              <div
                key={idx}
                className="p-6 bg-[#080B10] border border-white/10 space-y-3 hover:border-[#0099FF]/40 transition-all"
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] text-[#0099FF] font-bold tracking-wider uppercase">
                    0{idx + 1} / {way.label}
                  </span>
